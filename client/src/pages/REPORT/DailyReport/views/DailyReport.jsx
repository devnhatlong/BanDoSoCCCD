...
export const DailyReport = () => {
    ...
    const [reportData, setReportData] = useState({
        ...
        // Section 1.1 - Liên quan đến An ninh Biên giới, biển đảo
        securityBorderContent: '',
        
        // Section 1.2 - An ninh trên các lĩnh vực  
        securityFieldsContent: '',
        
        // Section 2 - Tình hình trật tự an toàn xã hội
        socialOrderContent: '',
    ...
    // Fetch server date khi component mount
    useEffect(() => {
        const fetchServerDate = async () => {
            try {
                // Có thể gọi API để lấy server date hoặc dùng client date
    ...
    const draftMutation = useMutationHooks(
        async ({ reportData, annexData }) => {
            ...
            if (reportResult.success) {
                ...
                const annexDataWithReportId = {
                    ...annexData,
                    dailyReportId: reportResult.data._id
                };
                
                const annexResult = await dailyReportAnnexService.createDailyReportAnnex(annexDataWithReportId);
    ...
    const handleSaveDraft = () => {
        if (!hasAnyData()) {
            message.warning('Vui lòng nhập ít nhất một trường dữ liệu trước khi lưu!');
            return;
        }
        ...
        const reportDataWithStatus = {
            ...reportData,
            serverDate: moment(serverDate).format('YYYY-MM-DD HH:mm:ss'), // Thêm server date
            departmentId: user.departmentId, // Thêm department ID
